...
var (
	...
	GoLangTmpl     LangTmpl = LangTmpl{
		template.FuncMap{"Mapper": mapperF,
			"Type":       typestring,
			"Tag":        tag,
			"UnTitle":    unTitle,
			"gt":         gt,
...
func mapperF(name string) string {
	objName := mapper.Table2Obj(name)
	objName = strings.ReplaceAll(objName, "id", "ID")
	objName = strings.ReplaceAll(objName, "Id", "ID")

	objName = strings.ReplaceAll(objName, "url", "URL")
	objName = strings.ReplaceAll(objName, "Url", "URL")
	return objName
}

func structnote(name string) string {
	objName := mapperF(name)
	note := structNoteMap[name]
	return fmt.Sprintf("// %v %v", objName, note)
}
